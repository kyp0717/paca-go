...
type PacaConn struct {
	conn    websocket.Conn
	msgType websocket.MessageType
}

// newNhooyrWebsocketConn creates a new nhooyr websocket connection
func CreatePacaConn(ctx context.Context, u url.URL) (conn, error) {
...
func (c PacaConn) close() error {
	return c.conn.Close(websocket.StatusNormalClosure, "")
}

// ping sends a ping to the client
func (c PacaConn) ping(ctx context.Context) error {
...
func (c PacaConn) readMessage(ctx context.Context) (data []byte, err error) {
	_, data, err = c.conn.Read(ctx)
	return data, err
}

// writeMessage writes a single message
func (c PacaConn) writeMessage(ctx context.Context, data []byte) error {
